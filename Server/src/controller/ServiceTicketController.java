...
public class ServiceTicketController extends ControllerBase {
	public String list() {
		String msg = null;
		List<ServiceTicket> cards = GenericHelper.GetResult(session,
				"From ServiceTicket");
		msg = SUCCESS;
	...
	public String listMemberShipCardID(Integer MemberShipCardID) {
		String msg = null;
		List<ServiceTicket> tickets = GenericHelper.GetResult(session,
				"From ServiceTicket where MemberShipCardID=\'"
						+ MemberShipCardID + "\'");
		msg = SUCCESS;
	...
	public String listTechnicianID(Integer TechnicianID) {
		String msg = null;
		String hql = "from ServiceTicket s,DedicatedService d where s.dedicatedService=d.id and d.technician=\'"
				+ TechnicianID + "\'";
		List<ServiceTicket> tickets = GenericHelper.GetResult(session, hql);
		msg = SUCCESS;
		return new ResponseBuilder(msg, tickets).toString();
	...
	public String Cancel(Integer ID) {
		...
		try {
			ServiceTicket ticket = (ServiceTicket) session.get(
					ServiceTicket.class, ID);
