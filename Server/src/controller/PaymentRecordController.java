...
public class PaymentRecordController extends ControllerBase {
	...
	public String list()
	    {
	        ...
	        GenericHelper.GetResult(session,"From PaymentRecord");
	        msg=SUCCESS;
    ...
    {
        String msg=null;
        String hql="from PaymentRecord p,Technician t where p.employee=t.id and t.employee=\'"+ EmployeeID +"\'";	       
        List<PaymentRecord> tickets=
	 ...
	 public String listPaymentMethodID(Integer PaymentMethodID)
	    {
	        String msg=null;
	        List<PaymentRecord> tickets=
	    	        GenericHelper.GetResult(session,"From PaymentRecord where paymentMethod=\'"+ PaymentMethodID +"\'");
	        msg=SUCCESS;
	        return new ResponseBuilder(msg,tickets).toString();
	 ...
	 public String create(Integer PaymentMethodID,Integer EmployeeID,Integer ServiceTicketID,Double amount)
	    {
		    PaymentRecord result=null;
		    PaymentMethod method = (PaymentMethod)session.get(PaymentMethod.class,PaymentMethodID);
		    ServiceTicket ticket = (ServiceTicket)session.get(ServiceTicket.class,ServiceTicketID);
		    Employee employee = (Employee)session.get(Employee.class, EmployeeID);	
	        Date date = new Date(0);  
	        Timestamp timeFinished = new Timestamp(date.getTime()); 
	        PaymentRecord record = new PaymentRecord(method,ticket ,employee,amount,timeFinished );
